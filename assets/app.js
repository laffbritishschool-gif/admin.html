...
function injectGlobalUiStyles(){
  ...
  const style=document.createElement('style');style.id='school-ui-settings-style';style.textContent=`
    ...
    .admin-profile-wrap{position:relative;margin-left:auto;z-index:120}.user-chip{cursor:pointer;user-select:none;display:flex;align-items:center;gap:9px;border:1px solid transparent;border-radius:14px;padding:5px 7px 5px 5px;transition:.2s;background:transparent}.user-chip:hover,.user-chip.open{background:#f5f8fd;border-color:#e1e8f2}.user-chip:focus-visible{outline:3px solid rgba(11,94,215,.16);outline-offset:2px}.avatar.avatar-square{border-radius:10px!important}.avatar.avatar-circle{border-radius:50%!important}.avatar.avatar-square img{border-radius:8px!important}.avatar.avatar-circle img{border-radius:50%!important}.user-chip .profile-chevron{width:15px;height:15px;display:grid;place-items:center;color:#8290a3;transition:.2s}.user-chip.open .profile-chevron{transform:rotate(180deg)}.profile-menu{position:absolute;right:0;top:calc(100% + 10px);width:235px;padding:8px;border:1px solid #e3e9f2;border-radius:16px;background:#fff;box-shadow:0 20px 45px rgba(20,43,82,.16);opacity:0;visibility:hidden;transform:translateY(-6px) scale(.98);transform-origin:top right;transition:.18s;pointer-events:none}.profile-menu.open{opacity:1;visibility:visible;transform:none;pointer-events:auto}.profile-menu-head{padding:10px 10px 9px;border-bottom:1px solid #edf0f4;margin-bottom:6px}.profile-menu-head strong,.profile-menu-head span{display:block}.profile-menu-head strong{font-size:11px;color:#172033}.profile-menu-head span{margin-top:3px;font-size:8px;color:#8b96a7;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.profile-menu-item{width:100%;display:flex;align-items:center;gap:10px;border:0;background:transparent;border-radius:10px;padding:10px;color:#344054;text-decoration:none;font:inherit;font-size:10px;font-weight:800;text-align:left;cursor:pointer}.profile-menu-item:hover{background:#f5f8fd;color:#0b5ed7}.profile-menu-item.logout-item{color:#b42318}.profile-menu-item.logout-item:hover{background:#fff3f3;color:#b42318}.profile-menu-icon{width:28px;height:28px;border-radius:9px;display:grid;place-items:center;background:#eef4ff;color:#0b5ed7;flex:0 0 auto}.logout-item .profile-menu-icon{background:#fff0f0;color:#c62828}.profile-menu-icon svg{width:15px;height:15px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.profile-menu-backdrop{position:fixed;inset:0;z-index:110;background:transparent;display:none}.profile-menu-backdrop.open{display:block}body.school-dark .user-chip:hover,body.school-dark .user-chip.open{background:#1a2940;border-color:#30405b}body.school-dark .profile-menu{background:#131d2d;border-color:#30405b;box-shadow:0 20px 45px rgba(0,0,0,.35)}body.school-dark .profile-menu-head{border-color:#26334b}body.school-dark .profile-menu-head strong,body.school-dark .profile-menu-item{color:#edf3ff}body.school-dark .profile-menu-head span{color:#9eacc4}body.school-dark .profile-menu-item:hover{background:#1a2940;color:#fff}.profile-menu-item:focus-visible{outline:2px solid rgba(11,94,215,.28);outline-offset:-2px}@media(max-width:520px){.profile-menu{right:-4px;width:220px}.user-chip>div{max-width:115px}.user-chip>div strong{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}}
...
export async function getAdminProfile(userId){
  const {data:profile,error:profileError}=await supabase
    .from('profiles').select('id,full_name,role,is_active,must_change_password,avatar_url,avatar_shape').eq('id',userId).maybeSingle();
...
export function mountShell(user){
  ...
  shell.innerHTML=`<aside class="sidebar" id="sidebar">
    <div class="brand"><img src="${escapeHtml(s.logo_url)}" alt="${escapeHtml(s.school_name)}"><div><strong>${escapeHtml((s.school_name||'Laff British').replace(/\s+Montessori.*$/i,'').trim()||'Laff British')}</strong><small>Montessori School</small></div></div>
    <nav>${nav.map((n,i)=>`<a class="nav-item ${i===active?'active':''}" href="${n[0]}"><span>${n[2]}</span><em>${n[1]}</em></a>`).join('')}</nav>
    <button class="logout" id="logout"><span>↪</span> Sign out</button>
  </aside>
  <main class="main">
    <header class="topbar">
      ...
      <div class="admin-profile-wrap">
        <button class="user-chip" id="profile-trigger" type="button" aria-haspopup="menu" aria-expanded="false" aria-controls="profile-menu">
          <span class="avatar ${user?.profile?.avatar_shape==='square'?'avatar-square':'avatar-circle'}"><img src="${escapeHtml(avatarUrl)}" alt="${escapeHtml(fullName)}" onerror="this.onerror=null;this.src='${escapeHtml(s.logo_url||DEFAULT_SCHOOL_SETTINGS.logo_url)}'"></span>
        ...
        <div class="profile-menu" id="profile-menu" role="menu" aria-label="Profile menu">
          ...
          <button class="profile-menu-item logout-item" id="profile-logout" type="button" role="menuitem">
            <span class="profile-menu-icon"><svg viewBox="0 0 24 24"><path d="M9 5H5v14h4"></path><path d="M13 8l4 4-4 4"></path><path d="M17 12H9"></path></svg></span>
            <span>Logout</span>
          </button>
        </div>
      </div>
    </header>
    <section class="page-content">${shell.dataset.content||''}</section>
  </main>`;
  ...
  const profileBackdrop=document.querySelector('#profile-menu-backdrop');
  ...
  document.querySelector('#profile-logout')?.addEventListener('click',()=>{setProfileOpen(false);signOut();});
